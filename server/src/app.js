...
app.get("/get_files", (req, res) => {
  fileModule.readDir("../static/files").then(response => {
    // get files then return to client
    res.send(response);
  });
});
...
app.post("/download_video", (req, res) => {
  console.log("\nGetting Information...");
  jamifyModule.download(req.body.link).then(vidInfo => {
    res.send(vidInfo); // return video info
  });
});

app.get("/download_info", (req, res) => {
  res.send(jamifyModule.downloadInfo()); // return data from function that grabs current download info
});
...
app.get("/posts", (req, res) => {
  ...
  Post.find({}, "title description", function(error, posts) {
    //find all posts documents, get said tables, callback
    if (error) {
      console.error(error);
    }
    res.send({
...
app.post("/add_post", (req, res) => {
  ...
  var new_post = new Post({
    //use model to create new document for db
    title: title,
...
app.put("/posts/:id", (req, res) => {
  ...
  Post.findById(req.params.id, "title description", function(error, post) {
    //find post by id (using params in url), get the title and description, pass found post to callback
    if (error) {
      console.error(error);
    }
...
app.delete("/posts/:id", (req, res) => {
  Post.remove(
    {
      //find post with id from params in url and delete it
      _id: req.params.id
    },
    function(error) {
      if (error) {
...
app.get("/post/:id", (req, res) => {
  //get individual post from server by searching id
  Post.findById(req.params.id, "title description", function(error, post) {
    if (error) {
      res.status(404).send(error);
    } else {
