...
router.get('/:videoId', auth, async (req, res) => {
    ...
    try {
        foundVideo = await Video.findById(req.params.videoId).populate(
            '_owner',
            ['-videos', '-password']
        );
    } catch (error) {
...
router.delete('/:videoId', auth, async (req, res) => {
    ...
    try {
        await Promise.all([
            fs.unlink(
                path.join(__dirname, '../static/files/', videoId + '.mp4')
            ),
            fs.unlink(
                path.join(__dirname, '../static/files/', videoId + '.jpg')
            )
        ]);
    } catch (error) {
        return res
...
router.put('/:videoId', auth, async (req, res) => {
    ...
    if (!owner)
        return res
            .status(401)
            .send(
                "Access denied. You don't have permission to edit this video."
            );
